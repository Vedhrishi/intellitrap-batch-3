...
export function describeAuthError(message: string | undefined): {
  ...
  if (raw.includes("signups not allowed") || raw.includes("signup is disabled"))
    return {
      message: "New registrations are currently closed on this workspace.",
      field: null,
  ...
  return { message: "Something went wrong. Please try again.", field: null, reason: "other" };
}

/** `vishnu@gmail.com` -> `v••••u@gmail.com`. Never store the raw address. */
